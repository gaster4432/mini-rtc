...
rem Try to set OP (opus) if not set
if "%OP%"=="" (
  if exist "%~dp0opus-mingw64\lib\libopus.a" (
    set "OP=%~dp0opus-mingw64"
  ) else if exist "%USERPROFILE%\Documents\opus-mingw64\lib\libopus.a" (
    set "OP=%USERPROFILE%\Documents\opus-mingw64"
  ) else (
    echo WARNING: opus not found automatically. Set OP environment variable to the opus root (contains lib\libopus.a) if the build fails.
    set "OP=%~dp0opus-mingw64"
  )
)
